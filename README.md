...
Not only will you learn the JavaScript language, but you will learn productive ways to understand and diagram your JS programs.  This process may feel slower and less practical than simply diving in and solving code challenges, but the long-term pay-offs will be more than worth it. Taking the time to learn ways of visualizing JS data management, and to practice thinking of your programs as computing theory-in-action will give you the mental toolkit necessary for testing, program design, applicaiton security, and applied CS.

A result of this approach is that topics will be spread out quite differently from what you may expect based on other JS courses. Take for example Primitives. To fully understand primitive types in JS you need to have a solid grasp of many language mechanisms (objects, context, inheritance, reference vs. value, implicit coercion, constructors, ...). Because JavaScript In Order is organized by _mechanisms_ & _concepts_ (ie. inheritance in primitives & constructors & classes) not _features_ (ie. everything about primitives, then everything about constructors), primitives will be split up over the entire length of JS In Order.  Each _feature_ of primitives will be covered when the _mechanism_ that enables that feature is reached, and when the _concepts_ necessary to understand why are covered. ie. you will learn about primitive methods (str.split(), str.toUpperCase()) only after understanding free variables and having learned how to diagram prototypical inheritance.

In addition to introducing the JS language in an unusual way - and in the spirit of providing useful mental models for understanding JS & progrogramming - JavaScript In Order also doubles as an applied introduction to abstract mathematics and CS.  You will learn to think of types as elements in sets, to think of your functions as maps between sets, to understand JS as computing machine, and to understand language features through replication.
...
1. [operators within types](#operators)  
1. [operators between types](#operators)  
